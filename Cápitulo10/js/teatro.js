...
function reservarPoltrona() {
    ...
    if (isNaN(poltrona) || poltrona > POLTRONAS || poltrona <= 0) {
        alert('Informe um número de poltorna válido')
        return
    }

    var ocupadas = []
    if (localStorage.getItem('teatroOcupadas')) {
        ocupadas.push(localStorage.getItem('teatroOcupadas').split(';'))
    }
...
btNovo.addEventListener('click', () => {
    if (confirm('Essa ação apagará todos os registros das atuais poltronas.')) {
        localStorage.clear()
